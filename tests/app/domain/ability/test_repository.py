...
class TestPokemonAbilityRepositorySave:
    """Test scope for save method"""

    @staticmethod
    @pytest.mark.asyncio
    async def test_pokemon_ability_repository_save_success(pokemon_ability_repository):
        """Should persist pokemon ability when data is valid"""
        pokemon_ability_data_order = 1

        pokemon_ability = await pokemon_ability_repository.save(
    ...
    @staticmethod
    @pytest.mark.asyncio
    async def test_pokemon_ability_repository_save_with_hidden_ability(
    ...
    @staticmethod
    @pytest.mark.asyncio
    async def test_pokemon_ability_repository_save_commit_error(
        ...
        with pytest.raises(Exception, match='Database error'):
            await pokemon_ability_repository.save(
                entity=PokemonAbility(
                    url='https://pokeapi.co/api/v2/ability/1/',
                    name='stench',
                    order=1,
...
class TestPokemonAbilityRepositoryFindBy:
    """Test scope for find one by method"""

    @staticmethod
    @pytest.mark.asyncio
    async def test_pokemon_ability_repository_find_by_not_found(pokemon_ability_repository):
        """Should return None when pokemon ability is not found"""

        result = await pokemon_ability_repository.find_by(order=999)
    ...
    async def test_pokemon_ability_repository_find_one_by_order_success(
        ...
        result = await pokemon_ability_repository.find_by(order=result_order)
