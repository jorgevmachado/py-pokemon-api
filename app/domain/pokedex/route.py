...
@router.post('/wild', response_model=PokedexPublicSchema)
async def get_wild_pokemon(service: Service, trainer: CurrentTrainer, params: GetWildPokemon):
    return await service.get_wild_pokemon(
        params=params, trainer_id=trainer.id, user_request=trainer.name
    )
