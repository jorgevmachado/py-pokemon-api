...
from app.domain.captured_pokemon.schema import CapturedPokemonPublicSchema, CapturedPokemonFilterPage, \
...
async def get_trainer_pokedex(
    ...
    service = TrainerService(session)
    return await service.list_pokedex(
        trainer_id,
        current_trainer,
        page_filter
    )
...
@router.get(
    '/{trainer_id}/pokemons',
    response_model=LimitOffsetPage[CapturedPokemonPublicSchema] | list[CapturedPokemonPublicSchema]
)
async def get_trainer_pokemons(
    ...
    service = TrainerService(session)
    return await service.list_captured_pokemon(
        trainer_id,
        current_trainer,
        page_filter
    )
...
@router.post(
    '/{trainer_id}/capture',
    response_model=CapturedPokemonPublicSchema
)
async def trainer_capture_pokemons(
    ...
    service = TrainerService(session)
    return await service.capture_pokemon(
        trainer_id,
        current_trainer,
        capture_pokemon
    )
